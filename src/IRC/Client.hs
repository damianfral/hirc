...
initIRCState :: Socket -> IRCClient -> IO IRCState
initIRCState socket client = do
  thread <- async $ do
  ...
  where
    sendMessage message = sendAll socket $ encodeUtf8 $ encodeMessage message
    sendLoop = forever $ do
...
withIRCClient :: HostName -> ServiceName -> (IRCClient -> IO a) -> IO a
withIRCClient hostname port run = bracket adquire release $ run . client
  where
    ...
    adquire = do
      ...
      initIRCState socket client
    release (IRCState {connectionThread, socket}) = do
